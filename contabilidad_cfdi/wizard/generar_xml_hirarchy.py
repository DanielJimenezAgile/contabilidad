...
class GenerarXmlHirarchyWizard(models.TransientModel):
        ...
        if self.tipo_de_reporte == 'Catalogo de cuentas':
            ...
            for line in report.account_ids.filtered(lambda x:not x.hide_line):
                ...
                if line.account_id:
                    ...
                    cuenta_sat = line.account_id.cuenta_sat or '' # line.account_id.group_id.cuenta_sat or ''
                    cuenta_tipo = line.account_id.cuenta_tipo or '' #line.account_id.group_id.cuenta_tipo or ''
    ...
    def action_validate_xml(self):
        ctx = self._context.copy()
        report = self.env['report_trial_balance_contabilidad_cfdi'].browse(ctx.get('active_id'))
